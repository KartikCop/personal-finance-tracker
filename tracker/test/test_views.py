...
def test_total_values_appear_on_list_page(user_transactions,client):
    ...
    respone = client.get(reverse('transactions-list'))
    assert respone.context['total_income'] == income_total
    assert respone.context['total_expenses'] == expense_total
    assert respone.context['net_income'] == net
...
def test_transaction_type_filter(user_transactions,client):
    ...
    respone = client.get(reverse('transactions-list'),GET_params)

    qs= respone.context['filter'].qs
...
def test_update_transaction_request(user, transaction_dict_params, client):
    ...
    # update the transaction via a POST request - mutate the dict 
